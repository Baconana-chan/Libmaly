...
pub fn detect_save_paths(
    ...
    if let Some(eng) = engine {
        match eng.to_lowercase().as_str() {
            ...
            "rpg maker" | "rpgmaker" | "rpg mv" | "rpg mz" => {
                detect_rpg_maker_saves(game_path, &mut paths)
            }
            _ => {}
        }
    }
...
fn detect_unity_saves(
    game_path: &str,
    _company_name: Option<&str>,
    _game_name: Option<&str>,
    paths: &mut Vec<SavePathInfo>,
) {
...
fn detect_renpy_saves(game_path: &str, _game_name: Option<&str>, paths: &mut Vec<SavePathInfo>) {
    ...
    #[cfg(target_os = "windows")]
    {
        if let Some(name) = _game_name {
            ...
            let save_path = PathBuf::from(appdata).join("RenPy").join(name);
...
fn detect_by_structure(game_path: &str, paths: &mut Vec<SavePathInfo>) {
    ...
    let save_folder_names = [
        "save", "saves", "Save", "Saves", "savedata", "SaveData", "SAVEDATA",
    ];
